...
<style>
    ...
    /* Hide main menu dropdown (3 dots) but NOT the hamburger sidebar toggle */
...
class YOLODetector:
    ...
    def _load_model(self):
        ...
        self.input_shape = meta.shape  # [batch, C, H, W]

        # Attempt to read class names from model metadata
        ...
        # Derive input size — fallback ke 416x416 sesuai training
    ...
    def _letterbox(self, frame: np.ndarray):
        """
        Resize frame ke input_h x input_w dengan mempertahankan aspek rasio
        menggunakan padding abu-abu (letterbox). Kembalikan gambar + metadata
        untuk membalik transformasi saat postprocess.
        """
        ...
        # Padding agar tepat input_w x input_h
        pad_w = self.input_w - new_w   # total padding horizontal
        pad_h = self.input_h - new_h   # total padding vertikal

        # Distribusikan padding secara merata (atas/bawah, kiri/kanan)
        top    = pad_h // 2
        bottom = pad_h - top
        left   = pad_w // 2
        right  = pad_w - left
        ...
        # Metadata untuk inverse-transform di postprocess
        meta = {"scale": scale, "pad_left": left, "pad_top": top}
        return padded, meta

    def preprocess(self, frame: np.ndarray):
        """
        Letterbox + normalize frame untuk YOLO input.
        Menyimpan metadata letterbox agar postprocess bisa mengembalikan
        koordinat ke ruang frame asli dengan presisi.
        """
        img_bgr, self._lb_meta = self._letterbox(frame)
        img_rgb = cv2.cvtColor(img_bgr, cv2.COLOR_BGR2RGB)
        img_norm = img_rgb.astype(np.float32) / 255.0
        img_chw = np.transpose(img_norm, (2, 0, 1))   # HWC -> CHW
        return np.expand_dims(img_chw, axis=0)          # add batch dim

    def postprocess(self, outputs, orig_h: int, orig_w: int):
        """
        Parse output YOLO dan kembalikan koordinat ke ruang frame asli.
        Mendukung YOLOv5 [1, anchors, 5+nc] dan YOLOv8/v9/v10 [1, 4+nc, anchors].

        Koordinat output model sudah dalam pixel di ruang letterbox (0..input_w/H).
        Kita kembalikan ke frame asli dengan:
        ...
        # ── YOLOv8 / v9 / v10: shape [1, 4+nc, num_anchors] ──
        # Setelah squeeze: [4+nc, anchors] → transpose → [anchors, 4+nc]
        # Ciri khas: kolom pertama (setelah transpose) adalah 4 koordinat, sisanya class scores
        # YOLOv5: shape [1, anchors, 5+nc]
        # Ciri khas: kolom ke-4 adalah objectness score, sisanya class scores

        if predictions.ndim == 3:
            predictions = predictions[0]   # squeeze batch dim

            # Deteksi format: YOLOv8 punya axis pendek di dim-0 (4+nc << anchors)
            is_yolov8 = predictions.shape[0] < predictions.shape[1]

            if is_yolov8:
                # YOLOv8/v9/v10: [4+nc, anchors] → [anchors, 4+nc]
        ...
        # ── Inverse letterbox → koordinat frame asli ──
        ...
        # Buang box yang kolaps setelah inverse transform
    ...
    def draw(self, frame: np.ndarray, boxes, scores, class_ids, class_names=None):
        """Draw bounding boxes with stylized rendering."""
        colors = [
...
def generate_class_colors(num_classes):
    np.random.seed(42)
    return {i: tuple(int(c) for c in np.random.randint(80, 255, 3)) for i in range(num_classes)}
...
def process_video(detector, video_path, class_names, skip_frames=1, max_frames=None):
    """Generator: yields (annotated_frame, stats_dict) per frame."""
...
with st.sidebar:
    ...
    st.markdown("**📂 Model**")
    model_file = st.file_uploader("Upload model .onnx", type=["onnx"], key="model_upload")
    ...
    if override_size:
        ...
        input_size = 416  # sesuai training

    st.markdown("---")
    st.markdown("**🏷️ Class Names** *(optional)*")
...
# Parse class names from sidebar
...
# Load / reload model when file or input_size changes
if model_file is not None:
    model_bytes = model_file.read()
    # Hash mencakup input_size agar reload otomatis saat size berubah
    model_hash = hash((model_bytes, input_size))
    if st.session_state.get("model_hash") != model_hash:
        with tempfile.NamedTemporaryFile(suffix=".onnx", delete=False) as tmp:
            tmp.write(model_bytes)
            tmp_model_path = tmp.name

        with st.spinner(f"Loading ONNX model (input {input_size}×{input_size})..."):
            try:
                detector_obj = YOLODetector(tmp_model_path, conf_thresh, iou_thresh)
                # Override input size dengan nilai dari sidebar (416×416 sesuai training)
                ...
                st.toast(f"✅ Model loaded — {input_size}×{input_size}", icon="🎯")
            except Exception as e:
                st.error(f"❌ Failed to load model: {e}")
                st.session_state.model_loaded = False
    else:
        # Update thresholds & input size tanpa reload penuh
...
if not st.session_state.model_loaded:
    st.info("⬅️ Upload `.onnx` model file from the sidebar to get started.")
...
if video_file and st.session_state.model_loaded:
    ...
    st.warning("⚠️ Please upload your ONNX model from the sidebar first.")

# ─── Image Detection ─────────────────────────
st.markdown("---")
st.markdown("### 🖼️ Quick Image Test")
img_file = st.file_uploader("Or test on a single image", type=["jpg", "jpeg", "png", "bmp", "webp"], key="img_test")
